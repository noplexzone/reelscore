...
function currentSession(req) {
  ...
  if (!raw) return null;
  return db.prepare(`SELECT s.token_hash,s.csrf_token,u.id,u.username,u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND julianday(s.expires_at) > julianday('now') AND COALESCE(s.last_seen_at,s.created_at) > datetime('now','-7 days') AND u.status='active'`).get(sessionTokenHash(raw));
}
...
function getFlow(req, state, provider) {
  const flow = db.prepare(`SELECT * FROM provider_flows WHERE state_hash=? AND provider=? AND consumed_at IS NULL AND julianday(expires_at) > julianday('now')`).get(sha256(String(state || "")), provider);
...
export async function finishFlow(flow, provider, identity, credentials, connection, req, res, admission = {}) {
  ...
  const result = db.transaction(() => {
    const liveFlow = db.prepare("SELECT * FROM provider_flows WHERE state_hash=? AND consumed_at IS NULL AND julianday(expires_at) > julianday('now')").get(flow.state_hash);
    ...
    if (!userId) {
      ...
      if (liveFlow.invite_hash) invite = db.prepare(`SELECT * FROM invites WHERE token_hash=? AND revoked=0 AND used_by IS NULL AND julianday(expires_at) > julianday('now')`).get(liveFlow.invite_hash);
